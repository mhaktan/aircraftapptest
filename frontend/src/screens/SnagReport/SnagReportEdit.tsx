        ...
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(form); }} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px 28px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                ...
                <div>
                  <TkInput mode="number" label="Reported By Id *" value={String(form.reportedById ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('reportedById', Number(v)))(e.detail)} />
                </div>
                <div>
                  <TkDatepicker label="Detection Date *" value={String(form.detectionDate ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('detectionDate', v))(e.detail)} />
                </div>
                <div>
                ...
                <div>
                  <TkInput mode="text" label="Crs Number" value={String(form.crsNumber ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('crsNumber', v))(e.detail)} />
                </div>
                <div>
                ...
                <div>
                  <TkInput mode="number" label="Line Mechanic Id" value={String(form.lineMechanicId ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('lineMechanicId', Number(v)))(e.detail)} />
                </div>
                <div>

...
export const DashboardScreen: React.FC = () => {
  ...
  const [api_block_3Data, setApi_block_3Data] = React.useState<Record<string, unknown>[]>([]);
  ...
  React.useEffect(() => {
    ...
    })).then((rows) => { setApi_block_3Data(rows); setApi_block_3Loading(false); });
  }, []);
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setApi_block_5Data(target != null ? (target as Record<string, unknown>) : json);
      } catch { /* ignore */ }
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/services/app/SnagReport/GetAll?MaxResultCount=5&Sorting=id desc`, { headers: getRequestHeaders() });
        // Token expired or invalid — clear auth and redirect to login
        if (res.status === 401) {
          ['_auth_token', '_bearer_token', '_refresh_token'].forEach(k => localStorage.removeItem(k));
          if (window.location.pathname !== '/login') window.location.href = '/login';
          ...
          <TkCard header="Kayıt Sayıları">
            <div slot="content" style={{ padding: 16 }}>
              <ResponsiveContainer width="100%" height={280}>
                  <BarChart data={api_block_3Data}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="name" />
                    <YAxis />
                    <Tooltip />
                    <Bar dataKey="value" fill="#379a79" />
                  </BarChart>
                </ResponsiveContainer>
            </div>
          </TkCard>
        ...
        <div style={{ gridColumn: 'span 6' }}>
          ...
          <TkCard header="Dağılım">
            <div slot="content" style={{ padding: 16 }}>
              <ResponsiveContainer width="100%" height={280}>
                  <PieChart>
                    <Pie data={api_block_4Data} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} label>
                      <Cell fill="#379a79" />
                      <Cell fill="#ff9800" />
                      <Cell fill="#4caf50" />
                      <Cell fill="#e91e63" />
          ...
          <div style={{ background: '#fff', borderRadius: 8, overflow: 'hidden', border: '1px solid #e8e8e8' }}>
            {api_block_5Loading ? (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {(Array.isArray(api_block_5Data) ? api_block_5Data : extractArray(api_block_5Data)).map((row: Record<string, unknown>, i: number) => (
        ...
        <div style={{ gridColumn: 'span 6' }}>
          <div style={{ background: '#fff', borderRadius: 8, overflow: 'hidden', border: '1px solid #e8e8e8' }}>
            {api_block_6Loading ? (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead><tr><th style={{ padding: '8px 12px', textAlign: 'left', borderBottom: '2px solid #e0e0e0', fontSize: 12, color: '#666' }}>ID</th><th style={{ padding: '8px 12px', textAlign: 'left', borderBottom: '2px solid #e0e0e0', fontSize: 12, color: '#666' }}>Ata Chapter</th><th style={{ padding: '8px 12px', textAlign: 'left', borderBottom: '2px solid #e0e0e0', fontSize: 12, color: '#666' }}>Title</th><th style={{ padding: '8px 12px', textAlign: 'left', borderBottom: '2px solid #e0e0e0', fontSize: 12, color: '#666' }}>Description</th><th style={{ padding: '8px 12px', textAlign: 'left', borderBottom: '2px solid #e0e0e0', fontSize: 12, color: '#666' }}>Severity</th></tr></thead>
                <tbody>
                  {(Array.isArray(api_block_6Data) ? api_block_6Data : extractArray(api_block_6Data)).map((row: Record<string, unknown>, i: number) => (
                    <tr key={i}><td style={{ padding: '8px 12px', borderBottom: '1px solid #f0f0f0', fontSize: 13 }}>{String(row['id'] ?? '')}</td><td style={{ padding: '8px 12px', borderBottom: '1px solid #f0f0f0', fontSize: 13 }}>{String(row['ataChapter'] ?? '')}</td><td style={{ padding: '8px 12px', borderBottom: '1px solid #f0f0f0', fontSize: 13 }}>{String(row['title'] ?? '')}</td><td style={{ padding: '8px 12px', borderBottom: '1px solid #f0f0f0', fontSize: 13 }}>{String(row['description'] ?? '')}</td><td style={{ padding: '8px 12px', borderBottom: '1px solid #f0f0f0', fontSize: 13 }}>{String(row['severity'] ?? '')}</td></tr>
                  ))}
                </tbody>
              </table>
            )}

...
using AircraftApptest.Authentication;
using AircraftApptest.Entities;
using AircraftApptest.EntityFrameworkCore;

namespace AircraftApptest.Web.Host.Controllers
{
        ...
        public ActionResult Authenticate([FromBody] AuthenticateModel model)
        {
            try
            {
                ...
                if (user.PasswordHash != null && user.PasswordHash.StartsWith("PBKDF2$"))
                    pwOk = AircraftApptest.Users.PasswordHelper.VerifyPbkdf2(model.Password, user.PasswordHash);
                else

...
namespace AircraftApptest.SnagReports
{
        ...
        ISnagReportAppService
    {
        ...
        protected override IQueryable<SnagReport> CreateFilteredQuery(PagedSnagReportResultRequestDto input)
        {
            return Repository.GetAll()
                .WhereIf(!input.Keyword.IsNullOrWhiteSpace(), x =>
                    ...
                    (x.CrsNumber != null && x.CrsNumber.Contains(input.Keyword)) ||
                ...
                .WhereIf(!input.CrsNumber.IsNullOrWhiteSpace(), x => x.CrsNumber != null && x.CrsNumber.Contains(input.CrsNumber))
                ...
                .WhereIf(input.ReportedById.HasValue, x => x.ReportedById == input.ReportedById.Value)
                .WhereIf(input.DetectionDate.HasValue, x => x.DetectionDate == input.DetectionDate.Value)
        ...
        public async Task<SnagReportDto> ChangeStatusAsync(long id, ChangeStatusInput input)
        {
            ...
            {
            ("Open", "InProgress", "Approve", false),
            ("InProgress", "PendingCRS", "Approve", false),
            ("InProgress", "InProgress", "Revise", false),
            ...
            if (input.Action == "Approve")
                await _flowEngine.TriggerAsync("start-investigation", "SnagReport", result);
            return result;
        }
